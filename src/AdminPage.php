		...
		(function () {
			...
			container.addEventListener('click', function (e) {
				...
				if (btn.classList.contains('vcip-move-up') && panel.previousElementSibling) {
					...
					if (confirm(<?php echo \wp_json_encode( __( 'Delete this scheme?', 'essential-verified-client-ip' ) ); ?>)) {
						panel.remove();
						reindex();
					}
				}
			...
			container.addEventListener('input', function (e) {
				if (e.target.classList.contains('vcip-scheme-name-input')) {
					...
					title.textContent = e.target.value || <?php echo \wp_json_encode( __( 'New Scheme', 'essential-verified-client-ip' ) ); ?>;
				}
			});
