...
final class UninstallTest extends TestCase
{
    ...
    private function defineUninstallStubs(bool $multisite): void
    {
        if (! \function_exists('is_multisite')) {
            eval('function is_multisite(): bool { return $GLOBALS["_vcip_test_is_multisite"] ?? false; }');
        }
        $GLOBALS['_vcip_test_is_multisite'] = $multisite;

        if (! \function_exists('delete_option')) {
            eval('function delete_option(string $option): bool { unset($GLOBALS["_vcip_test_options"][$option]); return true; }');
        }

        if (! \function_exists('get_sites')) {
            eval('function get_sites(array $args = []): array { return $GLOBALS["_vcip_test_sites"] ?? []; }');
        }

        if (! \function_exists('switch_to_blog')) {
            eval('function switch_to_blog(int $id): bool { $GLOBALS["_vcip_test_uninstall_blogs"][] = $id; return true; }');
        }

        if (! \function_exists('restore_current_blog')) {
            eval('function restore_current_blog(): bool { return true; }');
        }
    }
}
